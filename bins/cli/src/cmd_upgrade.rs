...
fn resolve_upgrade_trust_root(
    ...
    let state = storage::MaintainerState::load(data_dir).map_err(|e| {
        anyhow::anyhow!(
            "FATAL: cannot load the maintainer trust root from {}: {e}\n  \
             ...
             A file written by an older binary is migrated automatically, so this means the \
             file is damaged rather than merely old. Restore it from a backup, or remove it \
             deliberately and let the node re-derive the maintainer set from the chain.",
            data_dir.display()
        )
    })?;
...
///
/// WARNING (AUDIT-P2-012): `--network` is NOT a security boundary today. The mainnet and
/// testnet bootstrap arrays are BYTE-IDENTICAL, and the signed release message carries no
/// network term (`"{version}:{sha256(CHECKSUMS.txt)}"`), so a signature produced for a
/// testnet release verifies against the mainnet root and vice versa. The flag selects a
/// key array; it binds nothing. Adding a network term to the signed bytes invalidates
/// every already-published `SIGNATURES.json`, so it is deferred with its own rollout.
