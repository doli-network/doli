...
impl Node {
    ...
    pub(super) async fn on_peer_connected(&mut self, peer_id: PeerId) {
    ...
    pub(super) async fn on_peer_disconnected(&mut self, peer_id: PeerId) {
    ...
    pub(super) async fn on_new_block_event(
        &mut self,
        block: Block,
        source_peer: PeerId,
    ) -> Result<()> {
    ...
    pub(super) async fn on_peer_status(&mut self, peer_id: PeerId, status: StatusResponse) {
        debug!(
    ...
    pub(super) async fn on_status_request(
        &mut self,
        peer_id: PeerId,
    ...
    pub(super) async fn on_sync_request(
        &mut self,
        peer_id: PeerId,
        request: SyncRequest,
    ...
    pub(super) async fn on_sync_response(
        &mut self,
        peer_id: PeerId,
    ...
    pub(super) fn on_producers_announced(&self, remote_list: Vec<PublicKey>) {
    ...
    pub(super) fn on_producer_announcements(&self, announcements: Vec<ProducerAnnouncement>) {
    ...
    pub(super) fn on_producer_digest(
        &self,
        peer_id: PeerId,
        digest: doli_core::ProducerBloomFilter,
    ) {
        let gset = self.producer_gset.clone();
    ...
    pub(super) fn on_new_vote(&self, vote_data: Vec<u8>) {
    ...
    pub(super) async fn on_new_attestation(&mut self, data: Vec<u8>) {
    ...
    pub(super) async fn on_tx_announcement(&mut self, peer_id: PeerId, hashes: Vec<Hash>) {
        let mempool = self.mempool.read().await;
        let mut new_count = 0;
        for hash in &hashes {
            if !mempool.contains(hash) && self.pending_tx_announcements.record(*hash, peer_id) {
                new_count += 1;
            }
        }
        drop(mempool);
    ...
    pub(super) async fn on_tx_fetch_request(
        &self,
        peer_id: PeerId,
        hashes: Vec<Hash>,
    ...
    pub(super) async fn on_tx_fetch_response(
        &mut self,
        peer_id: PeerId,
