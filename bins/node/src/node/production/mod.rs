...
impl Node {
    ...
    pub(super) async fn try_produce_block(&mut self) -> Result<()> {
        ...
        // Derive bond counts from ProducerSet (epoch-stable, deterministic).
        //
        // CRITICAL: Previously used utxo.count_bonds() which reads UTXO state
        // that can differ between nodes by 1-2 blocks (gossip delay). Even 1
        // bond difference changes total_bonds → ticket = slot % total_bonds
        // gives different producer → "invalid producer for slot" → permanent
        // divergence. ProducerSet.bond_count is updated only via apply_block().
        let _slots_per_epoch = self.params.slots_per_epoch as u64;
        let active_with_weights: Vec<(PublicKey, u64)> = {
            let producers = self.producer_set.read().await;
            active_producers
                .into_iter()
                .map(|pk| {
                    let bond_count = producers
                        .get_by_pubkey(&pk)
                        .map(|info| info.bond_count.max(1) as u64)
                        .unwrap_or(1);
                    (pk, bond_count)
                })
                .collect()
        };
        // Note: Inactivity leak REMOVED from scheduler. producer_liveness is
        // LOCAL state that differs between nodes → scheduling disagreements.
        // Bond-weighted scheduler uses RAW on-chain bond counts ONLY.
