//! `test_network` integration tests — scale, gossip, scheduler and liveness.
//!
//! The harness these drive lives in `tests/common/mod.rs`. It is a SUBDIRECTORY
//! module, not a `tests/*.rs` file, so cargo compiles it into whichever test
//! binaries ask for it instead of turning it into a test target of its own.
//! Before INC-I-198 the harness and these tests shared one file that
//! `checkpoint_rotation.rs` also included, so every test below ran twice.
//
// OUTPUT CONTRACT: N/A — integration tests (INC-I-065)

// The harness is shared with other test binaries, so any single binary uses
// only part of it.
#[allow(dead_code)]
mod common;
...
#[tokio::test]
#[ignore] // Slow at this scale. Descriptor pressure is handled by the
          // INC-I-198 live-node budget, not by --test-threads=1.
...
#[tokio::test]
#[ignore] // Slow at this scale. NOT a ulimit problem: the ceiling is the fd
          // NUMBER (Darwin stdio `short _file`), which no rlimit can raise.
...
#[tokio::test]
#[ignore] // BLOCKED on macOS: RocksDB's fopen() wrapper caps at FD 32767 (FILE._file
          // is a short). This note was RIGHT and predates INC-I-198 by months; the
          // suite still went over the ceiling because nothing bounded the OTHER
          // tests' concurrent descriptor use. Measured cost is 28 fds per node, not
          // ~26, so the single-test ceiling is ~1170 nodes. 10000 exceeds the whole
          // INC-I-198 budget, so this test would take the entire budget, run alone,
          // and STILL fail on the fd number — hence still ignored on macOS.
          // Use test_realistic_gossip_10k_clustered instead (drops nodes between
          // clusters). On Linux this works — FILE stores the fd in an int.
