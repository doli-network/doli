...
| REQ-BOND-002 | Must | `inc_i_203_addbond_cap.rs::addbond_cap_verdict_is_the_gate_expression_minus_in_block` leg (a) **[M1 GREEN]**; `crates/mempool/tests/it/inc_i_203_admission_gap.rs::{req_bond_002_pool_admission_rejects_over_cap_addbond, req_bond_002_pool_rejection_carries_the_addbond_cap_grep_code}` **[M2 RED]** | §C admission path | `addbond_cap_verdict` @ `crates/mempool/src/addbond_cap.rs`, wired at admission by `Mempool::addbond_holdings_verdict` @ `crates/mempool/src/pool.rs` (called from `add_transaction`, mapped to `MempoolError::InvalidTransaction`) **[M2 GREEN]** |
| REQ-BOND-003 | Must | `crates/mempool/tests/it/inc_i_203_admission_gap.rs::{req_bond_003_pool_revalidate_evicts_a_resident_addbond_that_became_over_cap **[M2 RED]**, req_bond_003_pool_revalidate_keeps_a_resident_addbond_still_within_cap **[M2 GREEN, over-eviction guard]**}` | §C revalidate | `Mempool::addbond_holdings_verdict` @ `crates/mempool/src/pool.rs`, called from `revalidate` — evicts via the existing `to_remove` / `remove_transaction` path so inputs are released **[M2 GREEN]** |
| REQ-BOND-004 | Must | `inc_i_203_builder_gap.rs::{admission_expression_rejects_a_strict_subset_of_consensus, allowance_with_is_not_the_addbond_expression}`; `inc_i_203_addbond_cap.rs` leg (d) **[M1 GREEN]** | §D parity note | `addbond_cap_verdict` @ `crates/mempool/src/addbond_cap.rs` — CALLS `doli_core::validation::check_addbond_cap`; `allowance_with` is not reached. ONE verdict shared by the builder and by BOTH `pool.rs` call sites via `addbond_holdings_verdict` (`in_block_prior = 0`) **[M2 GREEN]** |
| REQ-BOND-005 | Must | `inc_i_203_builder_gap.rs::below_activation_height_over_cap_addbond_is_still_packed`; `inc_i_203_addbond_cap.rs` leg (b) **[M1 GREEN]**; `crates/mempool/tests/it/inc_i_203_admission_gap.rs::req_bond_005_pool_admission_unchanged_below_activation_height` **[M2 GREEN]** | §E | `addbond_cap_verdict` height guard @ `crates/mempool/src/addbond_cap.rs`; `WithdrawalParity::addbond_active` @ `production/withdrawal_holdings.rs`, fed the EXISTING `addbond_cap_enforcement_activation_height` @ `production/assembly.rs:190` (no new param) |
| REQ-BOND-006 | Must | `inc_i_203_builder_gap.rs::unavailable_holdings_fails_open_and_packs` (legs A+B); `inc_i_203_addbond_cap.rs` leg (c) **[M1 GREEN]**; `crates/mempool/tests/it/inc_i_203_admission_gap.rs::req_bond_006_pool_admission_fails_open_when_holdings_unavailable` **[M2 GREEN]** | §C fail-open constraint | fail-open lives in `addbond_cap_verdict` @ `crates/mempool/src/addbond_cap.rs` (non-`Found` ⇒ `Ok`); a missing builder entry maps to `Unavailable` in `allow_add_bond` @ `production/withdrawal_holdings.rs` |
| REQ-BOND-007 | Should | `bins/cli/tests/it/inc_i_203_addbond_headroom.rs` — 9 cells (`cli_add_bond_refuses_over_remaining_bonds_headroom`, `add_bond_refusal_states_current_count_cap_and_remaining_headroom`, `add_bond_accepts_the_request_that_exactly_fills_the_cap`, `add_bond_headroom_subtracts_pending_addbond_updates`, `add_bond_guard_is_wired_into_bonds_handler_before_any_utxo_is_consumed`, `producer_ledger_must_not_hardcode_the_bond_cap`, + 3 boundary/overflow cells) **[M3 RED → GREEN]** | §C CLI row | `addbond_headroom_check` @ `bins/cli/src/producer_ledger.rs`, called from `handle_add_bond` @ `bins/cli/src/cmd_producer/bonds.rs` after the registration check and before any `get_utxos` / signing / submit **[M3 GREEN]** |
...
| REQ-BOND-010 | Must | `inc_i_203_builder_gap.rs::admission_expression_rejects_a_strict_subset_of_consensus` + `bins/node/tests/addbond_cap_overflow.rs` unmodified **[M1 GREEN]** | §E | none (negative) — verified: `git diff --stat crates/core/src/network_params/` empty; no change to `crates/core/src/validation/` or `validation_checks.rs`; M2's only edit is `crates/mempool/src/pool.rs` +38/-0 (green-evidence: `inc-i-203-M2-test-green-evidence.txt`) |
