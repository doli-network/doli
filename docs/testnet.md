...
```bash
# ai1 (omegacortex):
ssh ilozada@198.51.100.1 "sudo systemctl stop doli-testnet-nt{1..5} doli-testnet-archiver && sleep 1 && \
...
| DNS | IP | Port | Node |
|-----|-----|------|------|
| `bootstrap1.testnet.doli.network` | 198.51.100.1 | 40303 | NT1 (ai1) |
| `bootstrap2.testnet.doli.network` | 198.51.100.1 | 40304 | NT2 (ai1) |

Both are relay-enabled and embedded as defaults in the binary — no `--bootstrap` flag needed.

### Mirror (ai2 — 198.51.100.2)
...
| DNS | IPs (round-robin) |
|-----|-------------------|
| `seed1.doli.network` | 198.51.100.1 + 198.51.100.2 |
| `seed2.doli.network` | 198.51.100.1 + 198.51.100.2 |
| `testnet.doli.network` | 198.51.100.2 |
| `archive.doli.network` | 198.51.100.2 |
