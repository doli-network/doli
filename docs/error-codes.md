...
Used in `bins/node/src/node/validation_checks/` (`mod.rs::validate_block_economics()`; withdrawal rows in `withdrawal_economics.rs`).
...
| `ECON_WITHDRAWAL_PAYOUT_EXCEEDS_NET` | RequestWithdrawal payout exceeds Σ penalized net of the spent Bond inputs + non-Bond inputs (INC-I-171; active from `inc_i_171_vesting_penalty_activation_height`, pinned at 133_640 on testnet and 418_000 on mainnet (both 2026-09-07); `u64::MAX` on devnet) | Reject block (Full/Light); warn only in Replay; rejected at mempool admission; skipped by the builder |
| `ECON_WITHDRAWAL_BOND_EXTRA_DATA_MALFORMED` | A spent Bond input's `extra_data` does not decode to a 4-byte slot (only reachable via snap-synced UTXO views; ERRTX007 blocks it in validated blocks) | Reject block, fail closed (INC-I-171, INV-VEST-013) |
| `ECON_VESTING_QUARTER_INVALID` | `vesting_quarter_slots` is 0 or exceeds `u32::MAX` (parameter fault, never a panic) | Reject block (INC-I-171) |
