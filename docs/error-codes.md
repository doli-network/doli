...
| `ECON_WITHDRAWAL_PAYOUT_EXCEEDS_NET` | RequestWithdrawal payout exceeds Σ penalized net of the spent Bond inputs + non-Bond inputs (INC-I-171; active from `inc_i_171_vesting_penalty_activation_height`, frozen `u64::MAX`) | Reject block (Full/Light); warn only in Replay; rejected at mempool admission; skipped by the builder |
