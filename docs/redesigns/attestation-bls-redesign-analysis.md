...
| **REQ-BLS-003** | Post-AH, `presence_root` MUST equal `presence_commitment(bitfield, aggregate)` = `BLAKE3(len_le(bitfield) ‖ bitfield ‖ len_le(aggregate) ‖ aggregate)`, hash-committing the aggregate with no header change. Both halves are length-prefixed (C9) so no byte can move across the split. | - [ ] Given a post-AH block, when the aggregate is stripped or altered by a relay, then `presence_root` no longer matches and the block is rejected.<br>- [ ] Given `BlockHeader::hash()`, when compared before/after, then the field list is unchanged.<br>- [ ] Given H < AH, then `presence_root == BLAKE3(bitfield)` exactly as today. | `block.rs` (unchanged), `assembly.rs`, `validation_checks.rs` · **M0 tests** `bins/node/tests/it/inc_i_178_m0_attestation_lock.rs`: `req_bls_003_ac2_presence_root_commitment_is_enforced_for_a_full_bitfield`, `..._for_a_sparse_bitfield`, `req_bls_003_ac2_stray_bits_beyond_producer_count_are_rejected_today`, `req_bls_003_ac3_empty_body_bitfield_bypasses_the_commitment_check_today`; `bins/node/tests/it/inc_i_178_m0_block_identity.rs`: `req_bls_003_ac3_block_header_hash_covers_the_header_only` · **M4 tests** `crates/core/tests/it/inc_i_178_m4_commitment.rs` (5 fns, all `req_bls_003_m4_*`); `bins/node/tests/it/inc_i_178_m4_commit.rs`: `req_bls_003_m4_post_ah_the_aggregate_verifies_over_exactly_the_set_bits`, `req_bls_003_m4_the_commitment_binds_the_pair_on_both_sides_of_the_gate`, `req_bls_003_m4_the_real_builder_commits_to_the_pool_at_the_activation_height` · **M4 impl** `crates/core/src/attestation/commitment.rs` (`presence_commitment`) @ `attestation/commit.rs`, `production/assembly.rs`, `validation_checks.rs` · **M4-resume tests** `bins/node/tests/it/inc_i_178_m4_preimage.rs` (6 fns: `req_bls_003_m4_post_ah_the_empty_body_root_is_the_canonical_commitment`, `req_bls_003_m4_post_ah_an_empty_aggregate_does_not_buy_the_legacy_preimage`, `req_bls_003_m4_post_ah_every_aggregate_shape_uses_the_same_commitment_rule`, `req_bls_003_m4_post_ah_a_legacy_root_does_not_match_the_expected_commitment`, `req_bls_005_m4_below_the_gate_the_root_is_still_blake3_of_the_bitfield`, `req_bls_003_m4_the_validator_expects_the_commitment_for_an_empty_aggregate`) |
| **REQ-BLS-004** | The verification key set MUST be reconstructed from the ONE canonical universe `[epoch_state.producer_list \| (active_at(h) \ base) sorted by pubkey bytes]`, via a single shared function used by the encoder, the aggregate validator, and `post_commit`. | - [ ] Given a block containing a mid-epoch-activated attester, when validated, then the aggregate verifies.<br>- [ ] Given the shared function, when grepped, then encoder, validator and `post_commit` all call it and none re-implements it.<br>- [ ] Property test: encode→decode round-trip identity at N ∈ {45, 200, 1000}. | new shared fn in `bins/node/src/node/`, `assembly.rs:410-432`, `post_commit.rs:37-57` · **M0 tests** `bins/node/tests/it/inc_i_178_m0_attestation_lock.rs`: `req_bls_004_ac3_encode_decode_identity_at_45_producers`, `..._at_200_producers`, `req_bls_004_ac1_mid_epoch_activated_attester_keeps_its_universe_index`, `req_bls_004_universe_prefix_property_matches_the_real_encoder` (N=1000 NOT covered at M0) · **M4 tests** `bins/node/tests/it/inc_i_178_m4_gate.rs`: `req_bls_004_m4_post_ah_the_three_consensus_sites_share_one_universe`, `req_bls_004_m4_post_ah_the_universe_is_duplicate_free_and_keeps_base_as_its_prefix` · **M4 impl** `bins/node/src/node/attestation/commit.rs` (`encoder_universe`/`post_commit_universe`/`stray_bit_universe_width`) @ `production/assembly.rs`, `apply_block/post_commit.rs`, `validation_checks.rs` · **M4-resume tests** `bins/node/tests/it/inc_i_178_m4_rewards_empty.rs` (4 fns, `req_bls_004_m4_*` / `req_bls_005_m4_pre_ah_*`); `bins/node/tests/m_rc9_silent_vec_regression.rs`: `inc_i_178_m4_post_ah_canonical_empty_credits_no_attendance`; `crates/rpc/tests/it/inc_i_178_m4_empty_detection.rs` (2 fns); `crates/rpc/src/methods/tests_inc_i178_m4_attestation_stats.rs`: `req_bls_004_m4_the_canonical_empty_is_not_a_block_with_attestations` |
| **REQ-BLS-005** | The whole change MUST be gated behind ONE new forward-only `NetworkParams` activation height. No `HardForkSchedule` entry. No `CURRENT_PROTOCOL_VERSION` bump. | - [ ] Given H < AH, when a new-binary node builds a block, then the bytes are identical to an old-binary node's (mixed-fleet hash equality test).<br>- [ ] Given the diff, when inspected, then `crates/updater/src/hardfork.rs` and `CURRENT_PROTOCOL_VERSION` are untouched.<br>- [ ] Mainnet default ships as `u64::MAX`; pinning is a separate decision-session. | `network_params/{mod,defaults,env_loader}.rs` · **M0 tests (AC-1 baseline only)** `bins/node/tests/it/inc_i_178_m0_block_identity.rs`: `req_bls_005_ac1_builder_bitfield_and_presence_root_are_deterministic`, `req_bls_005_ac1_prebuilt_block_bytes_are_byte_identical_within_one_slot`. AC-2/AC-3 are diff-inspection criteria, not testable at M0 (no AH field exists yet) · **M4 tests** `crates/core/tests/it/inc_i_204_m5_activation_height.rs`: `req_bls_005_m4_the_attestation_bls_gate_is_frozen_on_every_network`, `req_bls_005_m4_the_attestation_bls_gate_is_a_distinct_independently_settable_field`, `the_env_override_is_locked_on_mainnet_and_honoured_elsewhere` (BLS half); `bins/node/tests/it/inc_i_178_m4_gate.rs`: `req_bls_005_m4_the_gate_predicate_is_inclusive_at_the_activation_height`, `req_bls_005_m4_pre_ah_the_three_sites_keep_their_historical_divergence`, `req_bls_005_m4_pre_ah_the_builder_reproduces_every_frozen_golden_vector`, `req_bls_005_m4_no_protocol_bump_and_no_hardfork_entry`; `bins/node/tests/it/inc_i_178_m4_commit.rs`: `req_bls_005_m4_the_real_builder_is_byte_identical_one_block_below_the_gate` · **M4 impl** `crates/core/src/network_params/{mod,defaults,env_loader}.rs` (`inc_i_178_attestation_bls_activation_height`) @ `bins/node/src/node/init.rs` · **M4-resume tests** `bins/node/tests/it/inc_i_178_m4_preimage.rs`: `req_bls_005_m4_below_the_gate_the_root_is_still_blake3_of_the_bitfield`; `bins/node/tests/it/inc_i_178_m4_rewards_empty.rs`: `req_bls_005_m4_pre_ah_an_empty_bitfield_with_a_non_zero_root_still_aborts` |
| **REQ-BLS-006** | Attesters MUST dual-sign (Ed25519 + BLS) on the gossip wire, and this MUST ship in an EARLIER release than the AH-gated validator. | - [ ] Given a node on release N, when it attests, then `Attestation.bls_signature` is 96 bytes.<br>- [ ] Given a fleet probe, when run before the AH is pinned, then 100 % of active producers emit BLS-signed attestations.<br>- [ ] Given release N, when deployed rolling, then no block content changes (gossip-only). | `startup.rs:591-608`, `attestation.rs:74` · **M7.5 impl (AC-2 observable)** `bins/node/src/node/attestation/ingress.rs` (`BlsAttestVerdict::Valid` arm) + `bins/node/src/metrics.rs` (`doli_attestation_bls_valid_total`, `doli_attestation_bls_valid_attester_total{attester}`) · **M7.5 probe** `scripts/gauntlet-gs018.sh::_gs018_dual_check` · **M7.5 tests** `bins/node/tests/it/inc_i_178_m75_ingress_signal.rs` (13) |
| **REQ-BLS-007** | Verification MUST be reachable from the live apply path, and its execution MUST be observable. | - [ ] A negative test drives a forged bit through `Node::validate_block_for_apply` and asserts rejection.<br>- [ ] A counter metric increments once per post-AH block validated.<br>- [ ] The counter is non-zero on testnet within one epoch of the AH. | `validation_checks.rs`, `bins/node/src/metrics.rs` · **M7.5 impl (ingress observability)** `bins/node/src/metrics.rs` (`ATTESTATION_BLS_VALID_TOTAL` zero-initialised in `register_metrics()` = GS-018's capability marker, `ATTESTATION_BLS_VALID_BY_ATTESTER`) @ `bins/node/src/node/attestation/ingress.rs` · **M7.5 tests** `inc_i_178_m75_ingress_signal.rs`: `req_bls_007_the_fleet_wide_valid_counter_is_registered_from_process_start`, `req_bls_007_the_valid_arm_emits_the_grep_literal_at_debug_level`, `req_bls_007_the_valid_arm_writes_both_counter_families`, `req_bls_007_gs018_reads_both_metric_names` |
...
| **REQ-BLS-010** | Liveness MUST NOT regress: inability to build a valid aggregate MUST NOT block block production. | - [ ] Given a producer holding zero BLS-signed attestations, when its slot arrives, then it produces a block with an empty bitfield and empty aggregate, and the block is accepted.<br>- [ ] Chaos test: half the fleet stops BLS-signing post-AH; block production continues. | `assembly.rs`, `validation_checks.rs` · **M4 tests** `bins/node/tests/it/inc_i_178_m4_commit.rs`: `req_bls_010_m4_post_ah_zero_pooled_signatures_still_builds_a_commitment`, `req_bls_010_m4_the_parent_pool_is_cleared_at_the_epoch_boundary_only`, `req_bls_010_m4_a_rejected_block_does_not_clear_the_parent_pool` · **M4 impl** `bins/node/src/node/attestation/commit.rs` (empty-pool guard) @ `apply_block/post_commit.rs` (`parent_sig_pool.clear()`) |
...
| **REQ-BLS-014** | Reconcile the stray-bit validator denominator with the encoder width (P2 sub-risk). | - [ ] A test constructs an epoch where `producer_list` holds a producer inactive at H, and asserts an honestly-built block is accepted. · **M4 tests** `bins/node/tests/it/inc_i_178_m4_gate.rs`: `req_bls_014_m4_post_ah_one_width_replaces_the_recorded_disagreement`; `bins/node/tests/it/inc_i_178_m3_midepoch_exit.rs`: `inc_i_178_m3_midepoch_exit_rejects_an_honest_block` (un-ignored, gate-derived pair) · **M4 impl** `bins/node/src/node/attestation/commit.rs` (`stray_bit_universe_width`) @ `validation_checks.rs` · **M4-resume test** `bins/node/tests/it/inc_i_178_m3_midepoch_exit.rs`: `inc_i_178_m3_midepoch_exit_rejects_an_honest_block` (P2 now rebuilds the block in the post-AH shape before validating at the gate) |
