...
| stage_utxo_bytes | crates/storage/src/state_db/staging.rs | M4 | [F3] landing zone for a chunked transfer. M3's client still reassembles the whole canonical image in RAM (`VerifiedSnapshot.utxo_set` is a whole image), so `apply_snap_snapshot` still installs through `atomic_replace(into_pairs())`. M4 [F4] replaces that with per-chunk staging; the seam and its behaviour are locked NOW by `m3_staging_install.rs` so M4 cannot land it untested. |
| staged_utxo_len | crates/storage/src/state_db/staging.rs | M4 | Same seam: the observable that separates "staged" from "written to the live CF". |
| promote_staged_utxos | crates/storage/src/state_db/staging.rs | M4 | Same seam: the one `atomic_replace` promotion M4's install calls instead of `into_pairs()`. |
| clear_staged_utxos | crates/storage/src/state_db/staging.rs | M4 | Same seam: the SEPARATE post-batch write (Res-2). `promote_staged_utxos` already calls it; it is public so M4's abandon-download path can discard staging without promoting. |
