...
| staging_dir | crates/updater/src/staging.rs | M2 | INC-I-215 M1 ships the staging library; the node-side caller is M2 (`bins/node/src/updater/staged_apply.rs`), which resolves `{data_dir}/updates` before staging. Only `examples/inc_i_215_stage_probe.rs` (the milestone outcome probe) calls it today, and a probe harness is not a production caller. |
| target_dir_is_writable | crates/updater/src/staging.rs | M2 | INC-I-215 M1 ships the probe; M2's startup preflight and the `auto_apply` branch are the production readers. Deliberately landed with the stager so the two are reviewed and tested as one contract. |
| stage_release | crates/updater/src/staging.rs | M2 | INC-I-215 M1 ships the stager; M2's `auto_apply` staged branch calls it after `fetch_verified_release` when the install target is not writable. |
| staged_ready_version | crates/updater/src/staging.rs | M2 | INC-I-215 M1 ships the idempotency latch (REQ-215-005); M2 calls it before any download so the `ActivateEnforcement` transition cannot re-stage the same version. |
| read_staged | crates/updater/src/staging.rs | M3 | INC-I-215 M1 ships the reader; the consumer is M3's `doli upgrade --from-staged` (`bins/cli/src/cmd_upgrade.rs`), which reads the handoff back before re-verifying it. |
| verify_release_artifact_bytes | crates/updater/src/install_gate.rs | M3 | INC-I-215 M1 makes it the single L1-L4 implementation (`verify_release_artifact` already delegates to it, so the chain is live); the direct byte-level caller arrives with M3's `--from-staged` re-verification of the on-disk release. |
