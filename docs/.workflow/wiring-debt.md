...
| canonical_range | crates/storage/src/utxo/set.rs | M3 | [F2] shipped the streaming fold; the range API's production consumer is the [F3] chunked snap transport, which M3 wires. `canonical_digest` and `canonical_len` from the same fold are already wired (rpc/methods/stats.rs, utxo_size_monitor.rs). |
