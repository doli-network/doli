...
| [.workflow/architecture-reasoning.md](./.workflow/architecture-reasoning.md) | Design-synthesizer reasoning trace for `specs/vesting-penalty-consensus-architecture.md` (INC-I-171, 2026-09-05) -- conclusion-first convergence matrix, 4 contradictions resolved by source (age source: Bond `extra_data` IS stamped at apply, Failure Analyst F1 refuted; check site: core loop is post-mutation + Replay-muted + mempool-bypassed; fn home via Cargo graph; Replay vs REQ-VEST-005), spot-check log with file:line, conclusion-vs-report discrepancies, rejected proposals, radical tiebreaker (gap > 0.2), residual uncertainty. Earlier attestation-bls trace at the same path: git `98650be2`. |
...
| [improvements/attestation-bls-verification-improvement.md](./improvements/attestation-bls-verification-improvement.md) | Attestation/BLS verification cost/impact analysis (2026-08, pre-decision). **HISTORICAL and SUPERSEDED — do not implement from it.** Its SSF pick was Option B (embedded Ed25519 signatures, no BLS aggregation) and that option was **NOT taken**: the user chose **Option A — real BLS aggregate verification**, tracked as **INC-I-178** and designed in `specs/attestation-bls-architecture.md`. Kept as the record of the cost comparison that led to the Option A decision. |
