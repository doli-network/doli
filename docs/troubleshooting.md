...
**Symptom:** A node in snap sync logs `[SNAP_SYNC] F4 REFUSE (manifest root): … != quorum …` or `[SNAP_SYNC] F4 REFUSE (manifest anchor): manifest (…, h=…) != quorum anchor (…, h=…)`, retries alternate peers, and snap completes more slowly than before (or falls back to header-first).

**Cause / meaning:** This is the INC-I-143 F4 admission gate working, not an error. Before the fix, `handle_snap_snapshot` logged a served-root ≠ quorum-root mismatch at `info!` and ACCEPTED the snapshot anyway, and installed it at the serving peer's raw current-tip height. That is how seed1 spliced a forked anchor at a −1 height offset with a 45-block hole → permanent INTEGRITY −1. Two gates now guard manifest admission in `handle_state_manifest`: (1) the served `state_root` MUST equal the quorum-agreed root; (2) the manifest's `(block_hash, block_height)` MUST equal the quorum anchor recorded in `SyncPipelineData::SnapDownloading`. A failure of either increments `snap.integrity_refusals` and picks an alternate peer, then falls back to header-first if no alternates remain.

**What to do:** A few F4 REFUSE lines followed by a successful admit from an alternate peer is normal and correct — the node refused a manifest that did not match the quorum anchor and found one that did. Slower-but-correct is the intended trade. Persistent refusals across many peers mean genuine fleet tip-fragmentation (no quorum root) — investigate the fork itself, do NOT loosen the gate. Code: `crates/network/src/sync/manager/state_session.rs` (manifest gates), `snap_sync.rs` (quorum formation), `types.rs` (`integrity_refusals`).

**Retired log line (INC-I-231):** `[SNAP_SYNC] F4 REFUSE (manifest height): anchor (…, h=…) corroborated by 0/N` no longer exists. That gate counted peers whose stored `PeerSyncStatus` equalled the manifest anchor, but status is refreshed only by periodic status responses, so on a 10 s-slot chain it lags the tip and a CORRECT manifest corroborates 0/N. A fresh joiner refused four identical correct manifests, burned all three snap attempts and fell back to header-first. If you see this line, the node is running a binary older than the fix.

**Not a refusal:** `[SNAP_SYNC] Tip advanced at <peer>: requested (…, h=…) but manifest is (…, h=…) — re-collecting the quorum` (INC-I-231). The peer produced a block between quorum formation and the manifest. This consumes one snap attempt and re-collects the quorum at the new tip. It does NOT count an integrity refusal and does NOT blacklist the peer.
