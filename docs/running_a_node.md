...
| `doli_producer_bls_rotation_total` | INC-I-217 BLS key rotations installed at an epoch boundary. It counts the `RotateBlsKey` entries the boundary flush drained, so it moves only when a queued rotation is actually installed. It stays at 0 while `bls_key_rotation_activation_height` is frozen, because no rotation can enter a block |
