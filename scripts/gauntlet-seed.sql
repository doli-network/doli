...
INSERT INTO gauntlet_scenarios (scenario_id, name, description, incident_ids, assertions, scale_params, runner, status)
VALUES ('GS-021', 'bls-rotation-frozen-pre-activation',
  'Guards the pre-activation freeze of BLS key rotation (INC-I-217: TxType::RotateBlsKey = 32, its CLI, its RPC surface and its metric all ship in the binary, while bls_key_rotation_activation_height stays u64::MAX on mainnet, testnet and devnet -- shipped-but-frozen is exactly the state INC-I-075 showed can leak into consensus through a path nobody thought was reachable). ASSERTS (observational, read-only, state-neutral, testnet-only, no confirm-var, part of the default run): (0) PRECONDITION -- the gate is still u64::MAX for EVERY network default in crates/core/src/network_params/defaults.rs; no RPC exposes the height, so the checkout is the only source, and a pinned height SKIPs the whole scenario because rotations become legitimate at that point. (1) gs021-no-queued-rotation -- getProducers(active_only=false) on the local seed reports ZERO pendingUpdates entries with updateType == rotate_bls_key; one such entry below the activation height means a rotation reached the producer-set queue through a path the gate was supposed to close. It SKIPs on an unreachable or unparseable node and on an empty producer set. (2) gs021-rotation-metric-zero -- doli_producer_bls_rotation_total, written only by the epoch-boundary flush arm of apply_block/state_update.rs, reads 0 on every /metrics endpoint that exposes it; a build older than M9 exposes no such series and SKIPs. (3) gs021-below-ah-refused -- the node answers [ERRTX-ROT002] below the height. This one ALWAYS SKIPs today: proving it live needs a real rotation transaction, the CLI has no offline dry-run path (bins/cli/src/cmd_producer/rotate.rs), and a gauntlet scenario never writes to a chain to prove a refusal; the check re-arms itself when a dry-run flag lands. NOTHING in this scenario writes: one JSON-RPC read, N /metrics reads, two file reads.',
  json('["INC-I-217","INC-I-162","INC-I-178"]'),
