...
| **Dependencies** | `sqlite3`, `python3`, `curl`, live testnet (`scripts/testnet.sh start all`), `scripts/gauntlet-collect.py`. **Optional:** an authenticated `gh`, `jq`, `git`, and the `doli` CLI — GS-015 needs all four and SKIPs (never fails) without any of them; GS-017 needs a resolvable `doli` CLI plus `$HOME/testnet/keys/producer_*.json` and `$HOME/testnet/logs/n*.log`, and SKIPs without them. |
...
- `scripts/gauntlet-gs017.sh` — GS-017 "over-cap AddBond refused" (INC-I-203). Sourced by the runner and registered in `scripts/gauntlet-seed.sql`; **observational, chain-read-only, state-neutral, part of the DEFAULT run, no confirm-var**. Replays the incident shape: a producer holding 1 bond asked for 3000 more, the CLI built, signed and submitted it, and the toxic AddBond sat in 13 of 18 mempools poisoning block assembly — every scheduled leader that packed it discarded its own block and lost the slot. Three assertions: `gs017-cli-refuses-before-signing` invokes `producer add-bond` at **exactly headroom+1** bonds and requires a non-zero exit carrying a cap/headroom refusal (the M3 client text) or `ADDBOND_CAP_EXCEEDED` (the M2 node text) **and** no `addbond` left resident on the submitting node; `gs017-no-addbond-residency` sweeps RPC 8500-8517 for a resident `addbond` (a port with no RPC is tolerated, never a failure); `gs017-no-cap-poison-in-window` counts `[BLOCK_POISON]` lines carrying `ADDBOND_CAP_EXCEEDED` as a **delta** against the runner's per-node log byte offsets (`$NODECFG`, else `$GS017_OFFSETS`, else a snapshot taken on the spot) — every log still holds pre-fix events, so an absolute count would be red forever. **The count is derived from the live `bondCount`, never hardcoded:** any count at or below the headroom is one the node *accepts*, and would bond real funds on an unattended default run. **Assertion 1 exercises the CLIENT path (INC-I-203 M3, `bins/cli/src/producer_ledger.rs::addbond_headroom_check`) only.** The NODE admission path is covered by the 12 regression tests linked to `INV-BOND-002` plus the M2 live testnet evidence — do not try to bypass the CLI guard to reach it. Every precondition **SKIPs, never FAILs**: no `producer_*.json` wallet on disk, the mapped producer already at the cap (headroom+1 would then be a count the node accepts), no live producer RPC, no resolvable `doli`, no readable node log. Env: `GS017_KEYS_DIR`, `GS017_LOG_DIR`, `GS017_PORTS`, `GS017_SEED_PORT`, `GS017_MAX_BONDS`, `GS017_NETWORK`, `GS017_OFFSETS`, `DOLI_CLI`. Tests: `scripts/test_gauntlet_gs017.sh`.
