...
- `StateDb::set_rebuild_in_progress(target_height)` / `clear_rebuild_in_progress()` / `get_rebuild_in_progress() -> Option<(target_height, started_at_unix)>` — INC-I-156 AUDIT-P1-001 crash marker in `cf_meta` (`rebuild_in_progress`, 16B LE pair). Armed immediately before the destructive `clear()` of a legacy rebuild-from-genesis, disarmed only after that same call's `atomic_replace` succeeds — and also by a successful `apply_snap_snapshot` install, which replaces the whole set with a root-verified snapshot (AUDIT-P2-101). Ordinary WAL write (no fsync): marker and wipe share one sequential WAL, marker first. **The reader fails CLOSED (AUDIT-P3-103):** only a genuinely absent key returns `None`; a read `Err` or a present-but-not-16-byte value returns `Some((StateDb::REBUILD_TARGET_UNKNOWN /* u64::MAX */, 0))`, i.e. armed with an undecodable payload, which `rebuild_halt_reason()` and the startup log render as `target height UNKNOWN (marker unreadable)`. No version byte — any different-length future format already fails closed on the length check
...
- `SyncManager::request_genesis_resync(reason)` — gated genesis resync; 5 guards (floor, concurrent recovery, rate limit, snap availability, snap attempts); emergency reasons bypass the REFUSAL of guards 1 and 4. INC-I-204 M6: bypassing guard 4 no longer calls `enable_snap_sync()`, so under `--no-snap-sync` the request is honored with `snap.threshold` left at `u64::MAX`. `decision.rs:178`'s `snap_allowed` stays false, so the `needs_genesis_resync` flag cannot satisfy `should_snap` (`decision.rs:198`) and no snapshot is installed — recovery stays header-first and block data is preserved
...
### Sync — Force Reorg (operator escape, INC-I-204 M4.1)
- `ForceReorgDirective` — private to `SyncManager`: `{target, armed_at: Instant, armed_at_height}`. Memory-only by construction; never serialized, never persisted, erased by a restart (C9 / INC-I-196 self-brick filter)
- `ForceReorgPoll` — **3** variants: `Idle`, `Expired`, `Armed(Hash)`
- `FORCE_REORG_TTL_SECS` = `BRANCH_VERDICT_TTL.as_secs()` = 300 — wall-clock bound
- `FORCE_REORG_MAX_HEIGHT_SPAN` = `thresholds::MINOR_FORK_GAP_MAX` = 50 — height-span bound; a node that resumed advancing does not need rescuing
- `SyncManager::arm_force_reorg(target)` — REPLACES any pending directive; never queues. Stamps `armed_at_height` from `local_height`
- `SyncManager::force_reorg_target()` — non-consuming peek
- `SyncManager::poll_force_reorg(now, local_height)` — evaluates both bounds; clears the slot ONLY on expiry; `Armed` is non-destructive
- `SyncManager::consume_force_reorg()` — spends the single shot
...
- `RecoveryCoordinator::classify(ctx)` — pure read; gates on grace period / applied_since_rollback / cooldown, then evaluates Rule 1 + 1b (small-gap rollback, or `on_rollback_refused`'s bounded `SiblingFetch` when the finality guard refuses), Rule 2 (snap — `gap >= SNAP_SYNC_GAP_MIN` only; INC-I-204 M6 deleted the `rollback_exhausted` and `deep_fork_confirmed` triggers, so NO fork-evidence pattern can admit snap below that gap), Rule 3 (header-first, suppressed on a tip peers reject — C3), Rule 4 (genesis resync, `&& !wedged_shape` since M6 — unreachable in the wedge shape, which falls through to the terminal instead of wiping state), then the `Wedged` terminal; every cell of the state space terminates or returns `None` (REQ-FORK-010, C-6)
...
- MAX_BODY_SIZE = 2 MB
- ADMIN_METHODS (**14**): pauseProduction, resumeProduction, createCheckpoint, pruneBlocks, backfillFromPeer, enterRecoveryMode, exitRecoveryMode, bridgeFromArchive, getUtxoDiff, getStateSnapshot, getStateRootDebug, verifyChainIntegrity, repairArchiveFromPeer, forceReorgTo (auth-gated in HTTP layer via bearer token; getGuardianStatus is NOT admin-gated). Membership is pinned by `server::tests::force_reorg_to_is_admin_gated` + `the_admin_gate_is_only_ever_widened` (INC-I-204 M4.1)
...
- forceReorgTo — INC-I-204 M4.1 / REQ-FORK-012 (admin); params `{hash}`; parses BEFORE the SyncManager lookup so malformed input is -32602 and arms nothing; on success calls `SyncManager::arm_force_reorg(target)` on the handle the Node polls and returns `{status:"armed", target, ttlSeconds, maxHeightSpan}`. Arming is NOT the reorg — the decision lands on the next periodic tick and is read from `doli_force_reorg_outcomes_total{outcome}`. Handler in `methods/fork_escape.rs` (guardian.rs is over the module-size budget)
...
- handle_sync_request_bg() — free async function (#[allow(dead_code)]); **DEAD — not the live sync-request handler**; the live path is `validation_checks.rs::handle_sync_request` (see Validation section). Retained for reference only; 9 SyncRequest variants: GetHeaders, GetBodies, GetBlockByHeight, GetBlockByHash, GetHeadersByHeight (INC-I-012 F1, max 2000 headers), DirectAttestation (re-broadcasts), GetStateRoot (reads cache, falls back to fresh compute on miss — NOT the live GetStateRoot path), GetStateSnapshot (includes anchor header, epoch_bond_snapshot, epoch_accumulators; epoch_state_bytes=None in this path)
...
### Block Handling
- handle_new_block() — [APPLY_START] log; duplicate check (O(1)); equivocation check; if prev_hash != best_hash: genesis-hash drop guard, fork-id drop guard, height-occupied guard (fork choice: FORK_GUARD). A worse-slot same-height sibling (is_better=false) is NO LONGER dropped permanently — INC-I-143 F2 (wedge-escape) RETAINS it and re-evaluates it through the existing reorg engine (retain_sibling_and_try_escape); a better-slot sibling (is_better=true) signals fork recovery as before. Orphan Case A (normal genuine orphan: request current_height+1 from peer via ORPHAN_CHASE, UNCHANGED) vs Case B (fork orphan). Case B now has a sub-case: a descendant whose parent is in the eligibility-VALIDATED retained set (self.wedge_retained_tips) is NOT a genuine gap (parent chain is local, not on a peer) → routes to wedge-escape re-evaluation instead of ORPHAN_CHASE (which would request the wrong height, best_height+1); a parent that is merely cached or genuinely unknown falls through to the UNCHANGED ORPHAN_CHASE path (Stability Pillar). Then: fork_block_cache (cap 100); weight-based reorg via sync_manager.handle_new_block_weighted(); snap-sync ValidationMode selection (Light if snap_sync_height.is_some()); post-apply orphan drain; post-apply catch-up request; [APPLY_END] log
- retain_sibling_and_try_escape() — INC-I-143 F2 (wedge_escape.rs); the FORK_GUARD wedge-escape entry point. Retains a competing worse-slot sibling the height-occupied guard would otherwise drop and attempts a synchronous reorg onto its branch via the SAME reorg engine as handle_completed_fork_recovery (record_fork_block_weight → plan_reorg → execute_reorg); the finality guard (INV-SYNC-008) is preserved inside plan_reorg. FIX A: eligibility-gates every block against the epoch-frozen schedule (check_producer_eligibility) BEFORE it enters the reorg graph and requires a STRICT weight increase (equal-weight tie-break stays only on the peer-downloaded fork_recovery path — closes the hash-grind tip-displacement vector). FIX B: only descendants whose parent is in wedge_retained_tips may reroute here; that set is capped at WEDGE_RETAINED_CAP=64 to bound O(n²) amplification. FIX C: a cheap accumulated-weight short-circuit skips the deep plan_reorg walk when the sibling cannot out-weight the tip, and a would-be-winning branch that execute_reorg cannot land (evicted body) raises signal_stuck_fork instead of being silently dropped. NODE-LOCAL fork choice only — no block content / validity-rule change, no activation height (INC-I-075 Q1/Q2 = no)
- try_consume_force_reorg() — INC-I-204 M4.1 / REQ-FORK-012 (`node/force_reorg.rs`); the AUDITED operator wedge escape and the replacement for LB-4 (poison rollback bypassing the finality guard). Called from `run_periodic_tasks`; returns `None` iff nothing is armed. Order: read-lock peek → `poll_force_reorg` (expiry) → SameBranch (target is our tip or a canonical ancestor, cross-checked height_index↔hash_to_height) → resolve the target body (fork cache, then block store) else `UnknownTarget` with the directive RETAINED (arm-ahead is the intended use) → `plan_reorg_operator` else `PlanRefused` → resolve every planned body (fork cache, then block store), refusing with `ReorgDidNotLand` on the FIRST unresolvable one so neither check ever runs over a partial branch → corroboration ≥2/3 of local ProducerSet weight else `Uncorroborated` → `check_producer_eligibility` on every branch block else `Ineligible` (execute_reorg applies forward in ValidationMode::Light and skips this check, so the escape is the only thing standing there) → `execute_reorg` → verify `chain_state.best_hash == target` else `ReorgDidNotLand` (execute_reorg returns Ok(()) on four silent no-op paths). Crosses the finality MARKER on ReorgHandler; NEVER touches `FinalityTracker` (INV-FINALITY-001), never invokes snap sync, never routes through `rollback_one_block` (the BLOCK_POISON arm, INV-PROD-002), never discards the losing branch's bodies (REQ-FORK-011). Terminal outcomes spend the single shot; only `UnknownTarget` retains it. NODE-LOCAL, operator-triggered — no block content / validity-rule change, no activation height (INC-I-075 Q1/Q2 = no)
- `ForceReorgOutcome` — **8** variants + `ALL` + `label()`: `Executed`, `Expired`, `Uncorroborated`, `UnknownTarget`, `PlanRefused`, `SameBranch`, `Ineligible`, `ReorgDidNotLand`. `Ineligible` and `ReorgDidNotLand` are deliberately distinct from `PlanRefused`: a security refusal and a missing body demand different operator actions (the ambiguity M3 removed with `RollbackOutcome`)
- Corroboration (REQ-FORK-012 §4) — the DISTINCT block producers of the TARGET BRANCH's own signed headers, joined to the LOCAL `ProducerSet`, must satisfy `sum*3 >= total*2` (inclusive at exactly 2/3). Numerator and denominator share one unit: same `weight(height)` function, same `is_active()` filter, same height. NEVER peer count and NEVER peer-advertised `producer_pubkey` (unauthenticated, `protocols/status.rs`) — both are Sybil-forgeable and fail filter F7; NEVER attestation weight (trap T3)
- execute_reorg() — chain linkage validation; ensure_blocks_present(1, target_height.max(1)) / [FORK_GUARD_BACKFILL_REQUIRED] guard — range byte-identical to rollback_one_block's and to the one inside rebuild_producer_set_from_blocks, so the refusal precedes the marker arm and the UTXO clear even at target_height == 0, where a bare 1..=target_height would be a no-op (`ensure_blocks_present` returns Ok for low > high) and the reorg would arm a durable halt marker it then never disarms (INC-I-156 M2 QA F1); the guard is height-INDEX-only, so an index-dense/body-absent height passes it and is caught later by the fail-closed replay below — same body-gap residual recorded under rebuild_producer_set_from_blocks (INC-I-156 M2 review F1); genesis-boundary cached_genesis_producers invalidation; undo-based rollback (preferred): reverts UTXOs + restores ProducerSet from snapshot; legacy fallback (no undo data): arms the `rebuild_in_progress` marker, rebuilds UTXO and ProducerSet from genesis — the UTXO replay **fails closed** on an unreadable block (`?` + `ok_or_else`, INC-I-156 AUDIT-P1-002; it previously used `.ok().flatten()` and skipped silently); rebuild_producer_liveness(); atomic_replace() persistence, then disarms the marker (only if this call armed it); ValidationMode::Light for all new blocks; mempool.revalidate()

### Block Application
- apply_block() — snap sync guard (silently drops blocks at or below snap_sync_height); duplicate check with poisoned-block re-apply path; validation; tx processing loop; chain state update; finality tracking; mempool pruning; atomic StateDb commit with undo data (UNDO_KEEP_DEPTH = 100); emits [STATE_FP] diagnostics fingerprint (8 consensus-derived hashes)
- UNDO_KEEP_DEPTH = 100 — number of historical undo entries to retain (2× MAX_CUMULATIVE_ROLLBACK = 50; 10× deepest observed mainnet reorg). Defined in `crates/core/src/consensus/constants.rs`. Beyond this window `execute_reorg` falls back to `rebuild_from_blocks`.
...
### Production
- try_produce_block() — state-corruption gate (`rebuild_halt_reason()`, LAST_CORRUPT_WARNING rate-limited 1/min; refuses while the `rebuild_in_progress` marker is set), version enforcement (LAST_WARNING rate-limited 1/min), hard fork check (LAST_FORK_WARNING rate-limited 1/min), slot dedup, production gate (handle_production_authorization), peer-aware behind-network check (3 blocks if height < 10, else 5), bootstrap vs epoch scheduling, rank guard, propagation delay (1s / 500ms bootstrap), slashing protection via signed_slots_db.check_and_mark(), block content build, drain events, VDF, post-VDF stale parent re-check, behind-tip broadcast suppression (skip if net_tip >= height + 2 && peer_count >= 3), apply, broadcast, attest; on apply_block failure: delegates to handle_failed_self_apply() (INC-I-204 M4.2, `production/poison.rs`) — purge the failed block's mempool TXs FIRST, then ask the rollback door with `RollbackAuthority::ProductionSelfApply { failed_height }`, which rewinds only when the failed block IS the local tip; otherwise the published tip is kept and the aborted apply's in-memory residue is resynced from StateDb; returns Ok on every path except a mid-rewind rollback error, which propagates the original apply error
...
- rebuild_epoch_state_from_blocks(&mut self, target_height: u64) — reconstructs epoch_state from block history when undo data absent or pre-upgrade; 3-epoch attestation lookback (Fix #4A), accumulator rebuild from scan (Fix #4B), edge scan guard (Fix #4B-edge), mid-epoch accumulator replay (Fix #4C). INC-I-082: operating height is the **explicit `target_height` parameter** (no internal chain_state/state_db read — eliminates the reorg-vs-rollback caller-ordering divergence; all 6 call sites in rollback_one_block/execute_reorg pass the post-rollback target). Branch order is `epoch<=1` → `has_incomplete_history` (INC-I-054/INC-I-082 defect 1: ALWAYS wins over in-mem accumulators — sets snap_sync_height for Light validation + returns scheduling-filtered active set, never silently uses stale attested_sets) → else full block-scan (the `have_inmem_accum` shortcut was removed; full-sync nodes always block-scan → attestation-filtered, bit-identical to derive_at_boundary). Producer_list/active_list/bond_snapshot use `active_producers_for_scheduling_at_height` with the same INC-I-068 weight-filter gating as post_commit (INC-I-082 defect 3) — output converges to the canonical post_commit/derive_at_boundary result for all reachable inputs. DEPRECATED degraded-fallback: only fires on pre-upgrade/absent undo data; firing on post-upgrade blocks indicates a persistence bug
- rebuild_producer_set_from_blocks(&self, producers: &mut ProducerSet, target_height: u64) -> Result<()> — **dense pre-check FIRST, then clear, then replay** (INC-I-156 M2 / AUDIT-P2 R2): ensure_blocks_present(1, target_height.max(1)) is the first statement and, on a store with a **height-index** hole, refuses with a [FORK_GUARD_BACKFILL] error naming the first missing height while leaving the caller's ProducerSet byte-identical — count, pubkeys, per-producer bonds and pending_updates all unchanged. It was previously destructive-then-fallible: producers.clear() ran first and the replay loop then aborted at the first missing height, emptying a LIVE ProducerSet that the very next apply_block persisted. **The guard NARROWS the destroy-then-abort class, it does not close it** (INC-I-156 M2 review F1): ensure_blocks_present reads the height INDEX only (block_store/queries.rs:199-207) while the replay loop needs a BODY (get_block returns Ok(None) when cf_bodies lacks the hash, block_store/queries.rs:35-38) — so on an index-dense/**body-absent** store the guard returns Ok, producers.clear() still runs and the replay still aborts; that residual shape is production-constructible by **three** distinct writers (AUDIT-P3-211) — seed_canonical_index (block_store/writes.rs:230-238, writes height_index + hash_to_height + snap_horizon and no header and no body), set_canonical_chain (writes.rs:107-170, indexes canonical heights on HEADER presence alone: the tip-down walk reads get_header at :161 and writes both index CFs at :145-146 without ever consulting cf_bodies), and put_block (writes.rs:20-70, whose header :31 / body :42 / slot-index :46 writes are three separate un-batched put_cf calls, so a crash between :31 and :42 leaves a durable header with no body) — is NOT covered by any M2 test, and is deferred to REQ-I156-015 in §7 of docs/bugfixes/inc-i-156-p2-residual-guards-analysis.md (body-density guard variant, or the REQ-I156-012 scratch-set shape which subsumes it). The .max(1) ADDS an unconditional "block 1 must exist" requirement at target_height == 0, and this is the ONE input on which the guard is STRICTER than the loop it fronts, not a no-op: the replay range 1..=0 is empty, so the loop cannot fail and always returns Ok, whereas the guard's 1..=1 returns Err on a store with no block 1 — an intentional Ok→Err behavior change that carries forward the INC-I-152 pre-check's own block-1 requirement (rollback.rs:168-172) and keeps all three sibling guards' ranges byte-identical. For every target_height >= 1 the guard's range equals the loop's exactly and nothing admitted before is refused now. Only after the guard passes does it clear and fully replay producer state by iterating blocks 1..=target_height; the guard's range is identical to that loop's, so nothing admitted before is refused now; the function writes to no store (it only reads block_store); replicates genesis phase completion; applies deferred pending_updates at every epoch boundary and every block in epoch 0; calls process_unbonding after each block; does NOT apply remaining pending_updates if target_height is mid-epoch

### Rollback
- rollback_one_block(authority: RollbackAuthority) — callee-side authority contract on the single rewind door (INC-I-204 M4.2): `CoordinatorApproved { depth }` (periodic.rs ShallowRollback) is unguarded and byte-identical to pre-M4.2; `ProductionSelfApply { failed_height }` is permitted only when `local_height == failed_height` and otherwise returns RollbackOutcome::RefusedNotAuthorized before any mutation and before cumulative_rollback_depth moves — `apply_block` runs before broadcast, so a self-apply failure above the tip would retract this node's own published block; `ReorgPlan` / `WedgeEscape` are reserved for M6. O(1) when undo data present: reverts UTXOs (remove created, restore spent), restores ProducerSet from bincode snapshot (fallback to rebuild_producer_set_from_blocks on deserialization failure); O(chain) legacy fallback when no undo data: **refuses first** via ensure_blocks_present(1, target_height.max(1)) with a [FORK_GUARD_BACKFILL_REQUIRED] warning and NO state mutated — same guard, same range and same fail-closed shape as the sibling execute_reorg entry above (INC-I-152 AUDIT-P1-003), and the same height-INDEX-only scope: a body-absent height passes this guard and errors during the replay instead (body-gap residual recorded under rebuild_producer_set_from_blocks, INC-I-156 M2 review F1); only then arms the `rebuild_in_progress` marker (INC-I-156 AUDIT-P1-001), clears the UTXO set — a REAL clear on both backends since INC-I-156 R1 — and replays all blocks from genesis, disarming the marker after atomic_replace succeeds and only if this call armed it; refuses rollback to height 0 from established chain (Fix 3); refuses if cumulative_rollback_depth >= MAX_CUMULATIVE_ROLLBACK (Fix 4); invalidates cached_genesis_producers on genesis boundary crossing; calls rebuild_producer_liveness; updates chain_state and sync_manager (note_rollback_completed); calls clear_finality_if_below_tip EXCEPT under ProductionSelfApply — INV-FINALITY-001 forbids clearing the finality marker as a production-failure side effect (the guard already makes it a no-op there); atomic_replace() persistence; restores epoch_state from undo.epoch_state_snapshot if present, else rebuild_epoch_state_from_blocks; calls state_db.delete_chain_commitment()
...
- apply_snap_snapshot() — decodes ChainState, UtxoSet, ProducerSet ONCE via `verify_state_root_from_bytes()`, re-verifying the state root from that single decode; envelope/state consistency check; moves the decoded UTXO pairs into `StateDb::atomic_replace` via `UtxoSet::into_pairs()` (no `iter_all()` Vec materialisation); on the `atomic_replace` `Ok(())` arm ONLY: installs the in-memory 3-state, converts `utxo_set` to the state_db-backed variant (INC-I-118), derives the cached state root from that INSTALLED backend rather than from the pre-install decode (a wire hash proves transport, not storage) **and disarms the `rebuild_in_progress` marker** — a root-verified whole-set install repairs the truncation the marker describes, so leaving it armed would keep a self-healed node production-halted forever (INC-I-156 AUDIT-P2-101); a rejected snapshot or a failed `atomic_replace` installs nothing and the halt survives; mark_snap_synced(); seed canonical index; Option C: anchor header persistence; fast-path EpochState if epoch_state_bytes present; legacy reconstruction path (runs only if epoch_state_bytes absent or producer_list empty): bond snapshot (peer payload → persisted → UTXO fallback), epoch_accumulators_bytes, EpochState::derive_at_boundary(); sets snap_sync_height; set_store_floor() + record_block_applied_after_snap()
