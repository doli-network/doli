...
- handle_sync_request() — **LIVE sync-request handler** (`validation_checks.rs`); dispatched from `on_sync_request()`; handles 8 SyncRequest variants including GetStateSnapshot (M7: includes epoch_state_bytes). GetStateRoot delegates to `serve_state_root()` (State-Root Lazy Tier-0: lazy compute + memoize-on-serve). NOTE: both `handle_sync_request_bg()` (event_loop.rs) and the older inline `handle_sync_request` variants marked `#[allow(dead_code)]` are DEAD — this delegating handler is the sole production path.
