...
- apply_snap_snapshot() — re-verifies state root; deserializes ChainState, UtxoSet, ProducerSet; envelope/state consistency check; atomic state replacement; on the `atomic_replace` `Ok(())` arm ONLY: converts `utxo_set` to the state_db-backed variant (INC-I-118) **and disarms the `rebuild_in_progress` marker** — a root-verified whole-set install repairs the truncation the marker describes, so leaving it armed would keep a self-healed node production-halted forever (INC-I-156 AUDIT-P2-101); a rejected snapshot or a failed `atomic_replace` installs nothing and the halt survives; mark_snap_synced(); seed canonical index; Option C: anchor header persistence; fast-path EpochState if epoch_state_bytes present; legacy reconstruction path (runs only if epoch_state_bytes absent or producer_list empty): bond snapshot (peer payload → persisted → UTXO fallback), epoch_accumulators_bytes, EpochState::derive_at_boundary(); sets snap_sync_height; set_store_floor() + record_block_applied_after_snap()
