...
| BLS key-rotation TxType before pin | – | – | – | Y (C7) | – | N (Won't) → **superseded 2026-09-11** | 1 vs 1 | OPTION O5 (resolved: build) |
...
**O5 — BLS key rotation before/with the pin (Failure Analyst C7) vs accept in writing that key loss = `Exit` + re-`Registration` (loses `registered_at` seniority under INC-I-193 tiering; vesting penalty, itself unenforced per INC-I-171).** No `TxType` among 24 rotates a BLS key; `bls_pubkey` is written only at Registration apply (`tx_processing.rs:245`), genesis completion (`genesis_completion.rs:134`) and rebuild (`rewards.rs:1302,1550`). A rotation TxType is its own AH and its own incident (analyst REQ-BLS-021 Won't). **SUPERSEDED by `specs/bls-key-rotation-requirements.md` (2026-09-11, run 552): the "rotation TxType shipped" branch was chosen; REQ-BLS-021 "Won't" is formally withdrawn and precondition P4 below closes when that feature merges (not when it is pinned).** **This is a user decision with permanent economic consequences (F3); the design blocks neither path.** conf(0.62, measured) on the hazard; low-evidence on which path. **ANNOTATION 2026-09-11 (run 552): the branch this entry records as SUPERSEDED has now MERGED.** INC-I-217 M1-M11 shipped `RotateBlsKey` (type 32), its CLI, its RPC surface and its metric, all behind `bls_key_rotation_activation_height = u64::MAX`. The text above is kept as written: it is the decision history, not the current state. For the current state read P4 below and section 3.24 of `specs/protocol.md`.
...
| P4 | Key-rotation decision recorded (O5): rotation TxType shipped, or written acceptance that key loss = permanent removal | **CLOSED 2026-09-11 — the feature MERGED (INC-I-217 M1-M11, run 552).** `RotateBlsKey` (type 32) exists IN CODE behind `bls_key_rotation_activation_height`, which is `u64::MAX` on mainnet, on testnet and on devnet (`crates/core/src/network_params/defaults.rs`). The gate is frozen, so no rotation can enter a block today. **Still owed before the rotation height is pinned:** (a) a `MIN_PEER_PROTOCOL_VERSION` bump — this needs explicit user approval and is NEVER bundled with another change; (b) F6 — a snap-syncing node receives `pending_updates` inside the bincode `producer_set_bytes`, which is OUTSIDE `serialize_canonical`, so the state root does not cover a queued rotation. M8 replays each block through `apply_rotation` (`bins/node/src/node/rewards.rs`), which makes the REBUILD path deterministic, but it does not make the queue state-root-covered; (c) the SEC-010 mismatch census is OWED. The artifact cited here, `docs/.workflow/bls-mismatch-census-2026-09-11.md`, does not exist in this worktree, so the counts recorded earlier (2 mainnet producers mismatched and earning 0; 33 of 93 active latently exposed) are UNREPRODUCED and must be re-measured at pin time; (d) a GS-021 gauntlet pass. Pinning the height is a separate decision-session. |
