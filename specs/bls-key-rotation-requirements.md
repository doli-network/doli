...
- [ ] `Exit` then rotation: the rotation is a no-op with a `warn!`. `Register` then rotation: the rotation applies after the register. `Slash` then rotation: defined, no panic.
...
| REQ-ROT-014 | Should | (test-writer) | (architect) | `crates/wallet/src/wallet.rs`, `bins/cli/src/` |
