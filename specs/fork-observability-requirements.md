...
| REQ-FORKOBS-CLF-002 | ForkType enum has: `TipRaceNatural`, `TipRaceHighLatency`, `ProducerEquivocation`, `EpochBoundaryInvalid`, `PostSnapDeadTip`, `ValidationDisagreement`, `RollbackLoop`, `SnapSyncToMinorityFork`, `ChainBreakLoop` (added workflow #349), `Unknown`. **Per Skeptic Reframe 4 & REQ-FORKOBS-RETRO-003: `Unknown` carries `reason_unknown: String` and `evidence_event_ids: Vec<String>` so the bundle escalates to human review instead of misclassifying.** | Must | The enum is exhaustive. `Unknown` variant carries `reason_unknown` (non-empty) and `evidence_event_ids`. |
...
| REQ-FORKOBS-CLF-006 | **Workflow #349 (Phase 1.5):** Add rule (h) `ChainBreakLoop` between rules (d) and (e). Classifies the post-snap chain-break / recovery-churn pattern (INC-I-083 historical, n6 2026-05-20 live) as a typed variant carrying `chain_break_count`, `recovery_attempts`, `seconds_stuck`, `rollback_count`. Rule fires when ANY of: (sig_a) chain_break_count > 3; (sig_b) fork_block_received_count > 100 AND ratio fork/applied > 10; (sig_c) rollback_count > 10; (sig_d) recovery_attempts > 20. Precedence MUST be after (d) and before (e)/(f) — the live testnet proved rule (f) `TipRaceNatural` confidently mis-classifies stuck-state nodes (n6 returned `TipRaceNatural` / `normal_operation` while frozen at h=115219 with fleet at 115269). recommended_action=`restart_with_resync` with structured args naming the safe wipe scope (preserves wallet.json + producer.seed.txt). Additive enum variant — rules (a)–(g) keep their existing thresholds and bodies. Q1=NO/Q2=NO/Q3=YES (read-only classifier extension over already-emitted events; safe for rolling deploy). | Must | n6 live-captured JSON bundle fixture classifies as ChainBreakLoop (via signals b+c+d). INC-I-083 historical log fixture classifies as ChainBreakLoop (via signal a). All existing classifier tests still pass — rule (h) does not steal events from rules (a)–(d). |
