...
- ARCHITECTURAL: Three-site parity — mempool and builder call the same resolver + predicate; mempool gains `current_slot: u32` (`pool.rs:424-428`); builder passes the `current_slot` in scope (`assembly.rs:15`).
    ...
    Evidence (verified 2026-09-05, `docs/.workflow/design-verification.md`): the tier is monotone non-decreasing in age (`constants.rs:430-437`) and the builder's `current_slot` is the slot written to `block.header.slot` (`assembly.rs:377`, `block.rs:385`), so the BUILDER is the exact safety gate. The mempool today has NO slot: `add_transaction(&mut self, tx, utxo_set, current_height)` is height-only (`pool.rs:424-428`, `withdrawal_holdings.rs:26-32`), `ValidationContext` gets `current_time = 0` (`pool.rs:460`). `chain_state.best_slot` is NOT a safe admission slot: a reorg sets `state.best_slot = common_ancestor_slot` (`block_handling.rs:803, 875`) and `revalidate` EVICTS on any `Err` (`pool.rs:1345-1352`), so an honest withdrawal built for a higher slot would be dropped after a reorg. Design amendment (R1), as SHIPPED at M5: the mempool evaluates the bound at `Mempool::slot_watermark` — the highest slot the pool has been handed, which never regresses — and skips the arm while that watermark is 0; `revalidate` does not evaluate it at all, so a reorg can never evict a resident withdrawal on a vesting error. The watermark is deliberately the MORE PERMISSIVE reading: the tier is monotone, so a regressed `best_slot` would tighten the bound and refuse an honest exit that was legal one slot earlier. Over-rejection at admission is the recorded harm here, not the safe fallback (INC-I-057 stuck txs, INC-I-203 fails OPEN on `HoldingsLookup::Unavailable` for exactly this reason, INC-I-147 admission/apply divergence). Admission is liveness-only: an over-admitted tx is skipped by the builder, never included.
...
| Mempool `Mempool::withdrawal_verdict` (`pool.rs:321`) | `Mempool::slot_watermark` — the max of every `current_slot` handed to `add_transaction`/`revalidate`, raised before any early return; watermark 0 SKIPS the arm | `mempool::vesting_bound::vesting_bound_verdict`, same `[CODE]`; `revalidate` passes `vesting_lane: false`, so no vesting `Err` can reach the evict-on-`Err` loop (`pool.rs:1374`) | liveness-only; builder is the safety gate |
| Builder `WithdrawalParity::allow_vesting` (`:144`) | `current_slot` (`assembly.rs:15`), carried in `VestingGate` | same wrapper | exact |
| Shared wrapper home | — | `crates/mempool/src/vesting_bound.rs`, beside `addbond_cap.rs`: the INC-I-203 precedent already has `bins/node` call into `mempool` for a shared admission/builder verdict | — |
...
| INV-VEST-011 | The mempool slot is a non-regressing watermark and IS the evaluation input; the arm is skipped while it is 0; a vesting `Err` never evicts a tx in reorg `revalidate`. A payout legal at the watermark stays legal after the call slot regresses | watermark-0 skip test; watermark test (accept at the regressed slot, plus a bound+1 reject so the arm is not merely disabled); reorg test: tx valid at slot S stays in the pool after `best_slot` drops to the common ancestor |
...
| INV-VEST-013 | The bound FAILS CLOSED on a malformed Bond: a spent Bond input whose `extra_data` does not decode to a u32 slot makes the withdrawal invalid. Never reuse `bond_creation_slot().unwrap_or(0)` (`in_memory.rs:325`, `queries.rs:204`), which fails OPEN (slot 0 = fully vested). Snap-sync ingress (`fork_recovery.rs:305`) bypasses ERRTX007, so malformed Bonds can exist in a snap-synced UTXO view | unit: `spent_bonds` with a 3-byte `extra_data` → `Err`, not `bound = amount` |
...
| M5 | `feat(mempool,production): vesting bound at admission and build; thread a non-regressing slot watermark` | shared wrapper `crates/mempool/src/vesting_bound.rs` (INC-I-203 `addbond_cap.rs` shape); `add_transaction`/`revalidate` gain `current_slot` and raise `Mempool::slot_watermark`; the bound is evaluated AT that watermark, skipped while it is 0; `revalidate` passes `vesting_lane: false`, so no vesting `Err` can evict; builder passes `current_slot` through `VestingGate` (`assembly.rs:196`); INV-VEST-011/013 | INV-VEST-008 harness; slot-0 skip; reorg-keeps-honest-tx test; malformed-Bond fail-closed test |
...
**Testnet:** T1 M1-M5 merged; INV-VEST-007 replay to tip. T2 the live testnet has ONLY genesis bonds (all 0%) — rehearse AddBond -> RequestWithdrawal at Q1..Q4 (quarter = 6 h), honest and patched CLIs. T3 zero honest would-rejects across >= 1 quarter boundary and >= 1 reorg. T4 `vesting_quarter_slots` env-lock confirmed on all 18 nodes — M2 REMOVED the `DOLI_VESTING_QUARTER_SLOTS` override on every network (`env_loader.rs:215`), so this is a binary-version check, not a per-node `.env` edit. T5 pin >= 24 h above the tip.
...
|---|---|---|---|
| R1 mempool slot source / monotonicity | CLOSED at M5 (liveness, not consensus) | tier monotone (`constants.rs:430-437`), so a LOWER slot is STRICTER and would censor honest exits after every reorg; over-rejection at admission is the recorded harm (INC-I-057, INC-I-203, INC-I-147), so the watermark — the more permissive reading — is the evaluation input; residents are untouched because `revalidate` does not run the arm | admission evaluates at `Mempool::slot_watermark` and skips while it is 0; builder is the exact safety gate on `block.header.slot` (INV-VEST-011, M5). Residual, deliberate: admission may be MORE PERMISSIVE than the block gate by up to the reorg depth or the clock skew — bounded, and the builder is exact, so the cost is one wasted mempool slot the builder then skips |
...
| New: fail-open decode default | ADOPTED as filter | `bond_creation_slot().unwrap_or(0)` (`in_memory.rs:325`, `queries.rs:204`) returns "fully vested" on a malformed Bond; snap-sync ingress `fork_recovery.rs:305` bypasses ERRTX007 | INV-VEST-013: the bound fails closed on a malformed spent Bond |
