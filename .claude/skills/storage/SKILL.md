...
**`producer/info.rs`** — `ProducerInfo` impl: `new/new_with_bonds/new_with_prior_exit`, `is_active/can_produce`, `start_unbonding/is_unbonding_complete`, `slash/is_slashed`, `add_bonds/remove_bonds`, `selection_weight_at(height, audit_activation)` (AUDIT-PROD-001: subtracts delegated_bonds post-gate), ()` (FIFO, _from_bonds()` free fn mirrors it), `apply_withdrawal()` (auto-exit at bond_count=0), `weight_for_network/effective_weight_for_network`, `activity_status_for_network`, `has_governance_power_for_network`. — NOTE (INC-I-171 M1): `calculate_withdrawal{,_with_quarter}` and `calculate_withdrawal_from_bonds` were deleted; the consensus penalty lives in `crates/core/src/validation/vesting.rs`, input resolution in `producer/withdrawal_inputs.rs`.
