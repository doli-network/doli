...
pub(super) fn load_from_env(network: Network) -> NetworkParams {
    ...
    NetworkParams {
        ...
        security_audit_activation_height: if is_mainnet {
            defaults.security_audit_activation_height
        } else {
            env_parse(
                "DOLI_SECURITY_AUDIT_ACTIVATION_HEIGHT",
                defaults.security_audit_activation_height,
            )
        },
        ...
        // INC-I-217 rotation gate. Deliberately NOT env-overridable on ANY
        // network: it decides whether a user-submittable transaction is valid.
        bls_key_rotation_activation_height: defaults.bls_key_rotation_activation_height,
