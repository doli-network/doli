...
fn adjacent_discriminants_unaffected() {
    // TxType: 22 = Swap (active), 29-30 = tombstoned (B.2), 31 = ZKSettle
    assert!(
    ...
    assert!(
        ...
        "TxType 29 (was FractionalizeNft) must be tombstoned (B.2)"
    );
    assert!(
        TxType::from_u32(30).is_none(),
        "TxType 30 (was RedeemNft) must be tombstoned (B.2)"
    );
    assert!(
